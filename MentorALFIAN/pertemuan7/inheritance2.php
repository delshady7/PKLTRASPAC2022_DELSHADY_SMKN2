...
class Produk {
    ...
    public function __construct( $title = "judul", $writer = "penulis", $publisher = "penerbit", $price = 0, $pages = 0, $episode = 0 )
    {
        $this->judul = $title;
        $this->penulis = $writer;
...
class novel extends Produk {
    public function getIngfo()
    {
        $info = "Novel: {$this->getLabel()} | Published by {$this->penerbit} ({$this->harga}) - {$this->halaman} Halaman.";
        return $info;
    }
}
class komik extends Produk {
    public function getIngfo()
    {
        $info = "Komik : {$this->getLabel()} | Published by {$this->penerbit} ({$this->harga}) ~ {$this->panel} Episode.";
        return $info;
    }  
}
class Ingfo {
...
$produk3 = new komik("Questism", "PTJ", "Webtoon", "14 Coin/Episode", 0, 66);
